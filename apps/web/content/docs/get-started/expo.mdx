...
- `expo-file-system` - Local storage for offline event queuing
...
Events are queued locally using `expo-file-system` when offline. The queue automatically syncs when connection is restored.
